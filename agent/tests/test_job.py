...
def test_skip_missing_data(config, client, phase):
    ...
    config[f"{phase}_command"] = "/bin/true"
...
def test_skip_empty_provision_data(config, client, phase):
    """Test that phases are skipped when there is no command configured."""
    config[f"{phase}_command"] = ""
...
def test_config_global_timeout(config, client):
    """Test that timeout from device config is preferred."""
    config["global_timeout"] = 1
...
def test_job_output_timeout(tmp_path):
...
def test_config_output_timeout(config, client):
...
def test_no_output_timeout_in_provision(
    config, client, tmp_path, requests_mock
):
    ...
    config["provision_command"] = "bash -c 'sleep 12 && echo complete'"
...
def test_run_test_phase_with_run_exception(
    ...
    config["setup_command"] = "fake_setup_command"
    requests_mock.post(rmock.ANY, status_code=HTTPStatus.OK)
...
def test_wait_for_completion(client):
...
def test_get_device_info(client, tmp_path):
    ...
    assert all(device_info[key] == value for key, value in fake_device.items())


def test_post_device_info_in_reserve(config, client, tmp_path, requests_mock):
    ...
    config["reserve_command"] = "/bin/true"
    fake_job_data = {
        "global_timeout": 1,
    ...
    # Compare retrieved data with expected data
    assert all(
        device_info_request[key] == value for key, value in fake_device.items()
    )
...
def test_send_and_store_results(
    config, client, phase, tmp_path, requests_mock
):
...
def test_fail_to_send_results(config, client, phase, tmp_path, requests_mock):
    ...
    requests_mock.post(rmock.ANY, status_code=HTTPStatus.UNPROCESSABLE_ENTITY)
    requests_mock.get(rmock.ANY, status_code=HTTPStatus.OK)
    job = _TestflingerJob(fake_job_data, client)
...
def test_incremental_status_updates(config, client, tmp_path, requests_mock):
    """Test that cumulative status updates are sent and stored."""
    # Configure multiple phases
    config["provision_command"] = "/bin/true"
    config["test_command"] = "/bin/true"
    fake_job_data = {
        "global_timeout": 1,
    ...
    for req in reversed(requests_mock.request_history):
        try:
            req_data = req.json()
            if "status" in req_data and TestPhase.TEST in req_data["status"]:
...
def run_testcmds(config, job_data, client, tmp_path) -> str:
...
def test_run_test_phase_secret(config, client, tmp_path, requests_mock):
    requests_mock.post(rmock.ANY, status_code=HTTPStatus.OK)
    requests_mock.get(rmock.ANY, json={}, status_code=HTTPStatus.OK)
    ...
    log_data = run_testcmds(config, job_data, client, tmp_path)
    print(log_data)
...
def test_serial_log_to_endpoint(
    config, client, server_api, tmp_path, requests_mock
):
    """
    ...
    config[f"{phase}_command"] = "/bin/true"
    requests_mock.post(rmock.ANY, status_code=HTTPStatus.OK)
    return_value, exit_event, exit_reason = job.run_test_phase(phase, tmp_path)
