...
class TestClient:
    ...
    def test_check_restart_offline_priority_over_restart(
        ...
        agent.status_handler.update(
            comment="Offline for test", mode_transition=True
        )
    ...
    def test_agent_maintenance_processes_maintenance_jobs(
        self, agent, requests_mock, caplog, tmp_path
    ):
        """Test maintenance mode processes only maintenance jobs."""
        mock_check_offline = [
        ...
        with (
            ...
            patch.object(agent, "process_jobs") as process_jobs,
        ):
        ...
        process_jobs.assert_called_once_with(maintenance=True)
...
class TestStartupModePreservation:
    """Restart-time preservation of MAINTENANCE and OFFLINE modes.

    Restarting an agent must not silently end an active maintenance
    period nor bring an offline agent back online.  These tests cover
    the acceptance scenarios documented in MAINTENANCE_MODE_HANDOFF.md.
    """
    ...
    def base_config(self):
        ...
        testflinger_agent.configure_logging(cfg)
        yield cfg
        shutil.rmtree(tmpdir)

    def _make_agent(self, requests_mock, cfg):
        requests_mock.post(rmock.ANY, status_code=200)
        client = _TestflingerClient(cfg)
        return _TestflingerAgent(client)

    def _mode_posts(self, requests_mock):
        return [
            call.json()
            for call in requests_mock.request_history
            if call.method == "POST"
            and call.url == self.AGENT_DATA_URL
            and "mode" in (call.json() or {})
        ]

    def _queue_posts(self, requests_mock):
        return [
            call.json().get("queues")
            for call in requests_mock.request_history
            if call.method == "POST"
    ...
    def test_startup_preserves_maintenance_mode(
        self, requests_mock, base_config, caplog
    ):
        """If the server already marks the agent as MAINTENANCE, the
        agent must remain in maintenance across a restart – it must not
        transition to ONLINE / WAITING.
        """
        requests_mock.get(
            self.AGENT_DATA_URL,
            json={
        ...
        modes = [p.get("mode") for p in self._mode_posts(requests_mock)]
        assert AgentMode.MAINTENANCE in modes, (
            "Expected MAINTENANCE mode to be re-posted on startup; "
            f"got: {modes}"
        )
        assert AgentMode.ONLINE not in modes, (
            "Startup must not switch a maintenance agent back to ONLINE; "
            f"got: {modes}"
        )

    def test_startup_preserves_maintenance_reason(
        self, requests_mock, base_config
    ):
        """The administrator's maintenance comment must remain associated
        with the agent after a restart.
        """
        reason = "Replacing hardware"
        requests_mock.get(
            self.AGENT_DATA_URL,
            json={"mode": AgentMode.MAINTENANCE, "comment": reason},
        )
        ...
        maintenance_posts = [
            p
            for p in self._mode_posts(requests_mock)
            if p.get("mode") == AgentMode.MAINTENANCE
        ]
        assert maintenance_posts, "Expected a MAINTENANCE mode post"
        assert any(p.get("comment") == reason for p in maintenance_posts), (
            "Expected the maintenance reason to be preserved; "
            f"got: {maintenance_posts}"
        )

    def test_startup_maintenance_restricts_advertised_queues(
        self, requests_mock, base_config
    ):
        """A maintenance agent must not take new work.  On startup the
        maintenance queue must be the last-advertised queue set so only
        purpose-sent maintenance jobs can reach it.
        """
        requests_mock.get(
            self.AGENT_DATA_URL,
            json={
        ...
        queue_posts = self._queue_posts(requests_mock)
        assert queue_posts, "Expected queues to be posted during startup"
        assert queue_posts[-1] == [f"{self.AGENT_ID}_maintenance"], (
            "Expected the final queues post on startup to restrict to the "
            f"maintenance queue; got: {queue_posts}"
        )

    def test_startup_preserves_offline_mode(self, requests_mock, base_config):
        """If the server marks the agent OFFLINE, it must stay OFFLINE
        after a restart rather than transitioning to ONLINE.
        """
        requests_mock.get(
            self.AGENT_DATA_URL,
            json={"mode": AgentMode.OFFLINE, "comment": "set by admin"},
        )

        self._make_agent(requests_mock, base_config)

        modes = [p.get("mode") for p in self._mode_posts(requests_mock)]
        assert AgentMode.OFFLINE in modes, (
            f"Expected OFFLINE mode preserved on startup; got: {modes}"
        )
        assert AgentMode.ONLINE not in modes, (
            f"Startup must not switch an offline agent to ONLINE; got: {modes}"
        )

    def test_startup_online_default_when_no_prior_mode(
        self, requests_mock, base_config
    ):
        """With no prior mode on the server, the agent starts ONLINE /
        WAITING (existing behaviour).
        """
        requests_mock.get(self.AGENT_DATA_URL, json={})

        self._make_agent(requests_mock, base_config)

        modes = [p.get("mode") for p in self._mode_posts(requests_mock)]
        assert modes and modes[-1] == AgentMode.ONLINE, (
            f"Expected ONLINE on startup with no prior mode; got: {modes}"
        )

    def test_startup_online_when_server_mode_restart(
        self, requests_mock, base_config
    ):
        """RESTART is the server's request that we have now honoured;
        after a restart we resume as ONLINE.
        """
        requests_mock.get(
            self.AGENT_DATA_URL,
            json={"mode": AgentMode.RESTART, "comment": "please restart"},
        )

        self._make_agent(requests_mock, base_config)

        modes = [p.get("mode") for p in self._mode_posts(requests_mock)]
        assert AgentMode.ONLINE in modes, (
            f"Expected ONLINE after honouring a RESTART; got: {modes}"
        )
        assert AgentMode.MAINTENANCE not in modes
        assert AgentMode.OFFLINE not in modes
