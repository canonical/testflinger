...
def test_check_jobs_empty(client, requests_mock):
    requests_mock.get(rmock.ANY, status_code=HTTPStatus.OK)
    job_data = client.check_jobs()
    assert job_data is None


def test_check_jobs_with_job(client, requests_mock):
    fake_job_data = {
...
def test_check_jobs_with_restricted_queues(
    client, config, server_api, requests_mock
):
    restricted_data = {
        "restricted_to": {
...
def test_check_jobs_with_unrestricted_queues(
    client, config, server_api, requests_mock
):
...
def test_post_advertised_queues(client, requests_mock):
    """
    ...
    assert requests_mock.last_request.json() == {"test_queue": "test_queue"}


def test_post_advertised_images(client, requests_mock):
    """
...
def test_post_provision_logs(client, server_api, requests_mock):
    ...
    requests_mock.post(
        f"{server_api}/agents/provision_logs/{client.config['agent_id']}",
...
def test_transmit_job_outcome(client, server_api, requests_mock, tmp_path):
...
def test_transmit_job_outcome_with_error(
    client, server_api, requests_mock, tmp_path, caplog
):
    """
...
def test_result_post_endpoint_error(client, server_api, requests_mock, caplog):
    """
...
def test_result_get_endpoint_error(client, server_api, requests_mock, caplog):
    """
    Test that the client handles the case where the server returns
...
def test_attachment_endpoint_error(client, server_api, requests_mock, caplog):
    """
    Test that the client handles the case where the server returns
    ...
    job_id = str(uuid.uuid1())
    requests_mock.get(
        f"{server_api}/job/{job_id}/attachments",
        status_code=HTTPStatus.NOT_FOUND,
    )
...
def test_transmit_job_artifact(client, server_api, requests_mock, tmp_path):
    ...
    testflinger_json.write_text(json.dumps(testflinger_data))
    requests_mock.post(
        f"{server_api}/result/{job_id}/artifact",
        status_code=HTTPStatus.OK,
    )
    client.transmit_job_outcome(tmp_path)
...
def test_transmit_job_outcome_missing_json(client, tmp_path, caplog):
    """
...
def test_post_status_update(client, server_api, requests_mock):
    """
    ...
    job_id = str(uuid.uuid1())
    requests_mock.post(
        f"{server_api}/job/{job_id}/events",
        status_code=HTTPStatus.OK,
    )
    events = [
        {
...
def test_status_update_endpoint_error(
    client, server_api, requests_mock, caplog
):
    """
    Test that the client handles the case where the server returns
...
def test_get_agent_data(client, config, server_api, requests_mock):
    agent_data = {
    ...
    data = client.get_agent_data(config["agent_id"])
...
def test_agent_registration_and_cookie_workflow(
    ...
    requests_mock.post(
        ...
        headers={
            "Set-Cookie": (
                f"agent_name={config['agent_id']}; HttpOnly; SameSite=Strict"
            )
        },
    )
...
def test_check_jobs_without_agent_registration_reregisters(
    ...
    post_requests = [
        req for req in requests_mock.request_history if req.method == "POST"
    ]
    assert len(post_requests) == 1
    assert post_requests[0].json() == {"job_id": ""}


def test_check_jobs_request_exception(client):
    """
    ...
    with patch.object(client.session, "get", side_effect=network_error):
        with patch("testflinger_agent.client.logger") as mock_logger:
            with patch("testflinger_agent.client.time.sleep") as mock_sleep:
                result = client.check_jobs()
