...
@patch("testflinger_device_connectors.devices.multi.multi.copy_ssh_keys_to_devices")
@patch("time.sleep")
def test_multi_reserve(mock_sleep, mock_copy_keys):
    ...
    # Mock builtins.open to return our temp file when job_list.json is requested
    with patch("builtins.open", create=True) as mock_open:
        mock_open.return_value.__enter__.return_value.read.return_value = json.dumps(job_list)
    ...
    assert mock_print.call_count >= 5  # Multiple print statements in reserve method
