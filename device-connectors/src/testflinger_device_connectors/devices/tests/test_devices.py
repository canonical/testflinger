...
from testflinger_device_connectors.devices import DefaultDevice, copy_ssh_keys_to_devices, copy_ssh_key
...
class CopySshKeysToDevicesTests(unittest.TestCase):
    ...
    @patch("os.unlink")
    def test_copy_ssh_keys_to_devices_success(self, mock_unlink, mock_import, mock_copy):
        ...
        mock_import.assert_has_calls([
            call("key1", keyfile="key.pub"),
            call("key2", keyfile="key.pub")
        ])
    ...
    @patch("testflinger_device_connectors.devices.import_ssh_key")
    @patch("os.unlink")
    def test_copy_ssh_keys_to_devices_import_failure(self, mock_unlink, mock_import, mock_copy):
    ...
    @patch("testflinger_device_connectors.devices.import_ssh_key")
    @patch("os.unlink")
    def test_copy_ssh_keys_to_devices_copy_failure(self, mock_unlink, mock_import, mock_copy):
        ...
        mock_copy.assert_called_once_with("192.168.1.1", "testuser", key="key.pub")

    @patch("testflinger_device_connectors.devices.copy_ssh_key")
    @patch("testflinger_device_connectors.devices.import_ssh_key")
    @patch("os.unlink", side_effect=FileNotFoundError)
    def test_copy_ssh_keys_to_devices_file_not_found(self, mock_unlink, mock_import, mock_copy):
    ...
    @patch("testflinger_device_connectors.devices.import_ssh_key")
    @patch("os.unlink")
    def test_copy_ssh_keys_to_devices_empty_lists(self, mock_unlink, mock_import, mock_copy):
...
class DefaultDeviceReserveTests(unittest.TestCase):
    ...
    def test_reserve_with_ssh_keys(self, mock_sleep, mock_copy_keys):
        ...
        with patch("testflinger_device_connectors.get_test_opportunity", return_value=job_data), \
             patch("builtins.open") as mock_open:
            # Mock the config file read
            mock_open.return_value.__enter__.return_value = Mock()
            mock_open.return_value.__enter__.return_value.read.return_value = '{"device_ip": "192.168.1.10"}'
            
            with patch("yaml.safe_load", return_value={"device_ip": "192.168.1.10"}), \
        ...
        mock_copy_keys.assert_called_once_with(["key1", "key2"], ["192.168.1.10"], "testuser")
    ...
    def test_reserve_no_ssh_keys(self, mock_sleep, mock_copy_keys):
        ...
        mock_copy_keys.assert_called_once_with([], ["192.168.1.10"], "testuser")
    ...
    def test_reserve_default_timeout(self, mock_sleep, mock_copy_keys):
        ...
        mock_copy_keys.assert_called_once_with(["key1"], ["192.168.1.10"], "testuser")
