...
class HPEDevice(OEMDevice):
    ...
    def __init__(
        ...
        try:
            ...
            raise FirmwareUpdateError("BMC IP address is not valid") from err
        else:
            self.bmc_ip = bmc_ip
    ...
    def _install_ilorest(self):
        ...
        subprocess.run(
            install_cmd, shell=True, capture_output=True, check=False
        )
        rc, stdout, stderr = self.run_cmd("--version", raise_stderr=False)
        if rc == 0:
            logger.info("successfully installed %s", stdout)
    ...
    def _get_hpe_fw_repo(self):
        ...
        for spp in sorted(set(links), reverse=True):
