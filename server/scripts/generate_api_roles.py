...
# AGENT and ORCHESTRATOR are separate roles outside this hierarchy.
...
# Display columns: machine roles first, then the human hierarchy.
ROLE_DISPLAY_ORDER = (
    ServerRoles.AGENT,
...
def _role_group_key(method: str, path: str, roles: list) -> tuple:
    ...
    - whether the lowest (widest-audience) role is allowed.
    ...
    role_count = len(allowed)
    return (agent_first, role_count if not agent_first else -role_count)
...
def generate_roles_matrix(spec: dict) -> str:
    ...
    lines.append("   :widths: 8 32 10 8 10 10 8")
    lines.append("")
    lines.extend(
